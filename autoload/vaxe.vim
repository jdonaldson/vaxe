...
function! vaxe#HaxeComplete(findstart,base)
   ...
   if a:findstart
       ...
       let type = match(line, '\u\w*$') 
       echomsg 'period: '.period
       echomsg 'paren: '.paren
       echomsg 'type: ' .type
       if !(period || paren || type)
           return -1
       endif

       let maxdisplay = max([period+1,paren+1])
       echomsg 'maxdisplay: ' . maxdisplay
       let basecol = max([maxdisplay, type])
       echomsg 'basecol: ' . basecol
       return basecol
   else
...
function! s:ToplevelCandidates(base)
    let matches = readfile(expand("%:p"))
    echomsg 'file '. join(matches,', ')
    " call map(matches, 'matchstr(v:val,"\\v\\s*((\\@\\w*\\s*)*class|import|enum|typedef)\\s*' . a:base . '\\zs\\u\\w+")')
    call map(matches, 'matchstr(v:val,"\\v\\s*((\\@\\w*\\s*)*class|import|enum|typedef)\\s*' . a:base . '\\zs\\w+")')
    echomsg 'types: '. join(matches,', ')
    echomsg  'matchstr(v:val,"\\v\\s*((\\@\\w*\\s*)*class|import|enum|typedef)\\s*' . a:base . '\\zs\\w+"'
    call filter(matches, 'v:val != ""')
    echomsg 'matched: ' . len(matches)
...
function! s:DisplayCompletion(base)
    ...
    " skip this for now, basic completion is just fine
    " if match(a:base, "\u\w*$")
    "     return s:ToplevelCandidates(a:base)
    " endif
